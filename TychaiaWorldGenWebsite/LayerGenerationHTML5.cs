...
namespace TychaiaWorldGenWebsite
{
    public static class LayerGenerationHTML5
    {
        ...
        private static void SetColor(dynamic imageData, int x, int y, LayerColor c)
        {
            ...
            for (int x = 0; x < width; x++)
                for (int y = 0; y < height; y++)
                {
                    if (colors != null && colors.ContainsKey(data[x + y * width]))
                        SetColor(imageData, x, y, colors[data[x + y * (width)]]);
                    else
                        SetColor(imageData, x, y, m_UnknownColor);
                }
        }
