...
namespace Tychaia
{
    public class IsometricWorldManager : IWorldManager
    {
        ...
        protected void HandleRenderOfEntity(IGameContext gameContext, IRenderContext renderContext, IEntity a)
        {
            ...
            if (a is ChunkEntity)
            {
                ...
                if (!(gameContext.World is RPGWorld))
                    return;
                this.Octree = (gameContext.World as RPGWorld).ChunkOctree;
                if (this.Octree == null)
                    return;
                if (this.Chunk == null)
                ...
                if (RenderingBuffers.DepthBuffer != null && FilteredFeatures.IsEnabled(Feature.IsometricOcclusion))
                {
                    ...
                    this.m_OccludingSpriteBatch.DrawOccludable(
                        gameContext.Textures[ce.Image],
                        new Rectangle((int)(pos.X - ce.ImageOffsetX), (int)(pos.Y - ce.ImageOffsetY),
                            gameContext.Textures[ce.Image].Width, gameContext.Textures[ce.Image].Height),
                ...
                base.HandleRenderOfEntity(gameContext, a);
        }

        protected void PreBegin(IGameContext context)
        {
        ...
        protected void DrawTilesBelow(IGameContext gameContext, IRenderContext renderContext)
        {
            // Ensure we have a chunk manager to source chunks from.
            if (!(gameContext.World is RPGWorld))
                return;
            ...
            IEnumerable<RelativeRenderInformation> renders = this.GetRelativeRenderInformation(gameContext, this.Chunk);
            ChunkRenderer.LastRenderedCountOnScreen = renders.Count();

            // Render chunks.
            if (FilteredFeatures.IsEnabled(Feature.DepthBuffer))
            {
                gameContext.EndSpriteBatch();
                gameContext.Graphics.GraphicsDevice.SetRenderTarget(RenderingBuffers.ScreenBuffer);
                gameContext.StartSpriteBatch();
            }
            foreach (RelativeRenderInformation ri in renders)
            {
                ...
                if (tex != null)
                {
                    ...
                    if (FilteredFeatures.IsEnabled(Feature.DepthBuffer))
                        gameContext.SpriteBatch.Draw(tex, new Vector2(ri.X, ri.Y + zoffset), Color.White);
                    else
                    {
            ...
            {
                gameContext.EndSpriteBatch();
                gameContext.Graphics.GraphicsDevice.SetRenderTarget(RenderingBuffers.DepthBuffer);
                gameContext.StartSpriteBatch();
                foreach (RelativeRenderInformation ri in renders)
                {
                    ...
                    if (depth != null)
                    {
                        ...
                        if (FilteredFeatures.IsEnabled(Feature.DepthBuffer))
                        {
                            gameContext.SpriteBatch.Draw(depth, new Vector2(ri.X, ri.Y + zoffset), Color.White);
                        }
                    }
                }
            }
        ...
        protected void DrawTilesAbove(IGameContext gameContext, IRenderContext renderContext)
        {
            ...
            else
            {
                ...
                {
                    if (FilteredFeatures.IsEnabled(Feature.RenderingBuffers))
                        gameContext.SpriteBatch.Draw(RenderingBuffers.ScreenBuffer, Vector2.Zero, Color.White);
                }
