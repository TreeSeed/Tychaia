...
namespace Tychaia.ProceduralGeneration
{
    ...
    [FlowDesignerName("Zoom 2D Bundle")]
    public class AlgorithmZoom2DBundle : Algorithm<FlowBundle, FlowBundle>
    {
        [DataMember]
        ...
        public override void ProcessCell(IRuntimeContext context, FlowBundle[] input, FlowBundle[] output, long x, long y, long z, int i, int j, int k, int width, int height, int depth, int ox, int oy, int oz)
        {
            ...
            else
            {
                ...
                FlowBundle east = input[((i + 1) / 2 + ox + ocx_e) + (j / 2 + oy + ocy) * width + ((k) + oz + ocz) * width * height];
                ...
                switch (selected)
                {
                    ...
                    case 1:
                        ...
                        FlowBundle south = input[(i / 2 + ox + ocx) + ((j + 1) / 2 + oy + ocy_s) * width + (k + oz + ocz) * width * height];
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        FlowBundle southEast = input[((i + 2) / 2 + ox + ocx) + ((j + 2) / 2 + oy + ocy) * width + (k + oz + ocz) * width * height];
